...
public class OAuthRequest extends HTTPRequest {
    ...
    {
        if (key.startsWith(OAUTH_PREFIX) || key.equals(OAuthConstant.SCOPE) || key.equals(OAuthConstant.REALM))
        {
            return key;
        }
        else
        {
            throw new IllegalArgumentException(String.format("OAuth parameters must either be '%s', '%s' or start with '%s'", OAuthConstant.SCOPE, OAuthConstant.REALM, OAUTH_PREFIX));
        }
    }
    ...
    {
        return String.format("@OAuthRequest(%s, %s)", getHttpMethod(), getUrl());
    }
}
